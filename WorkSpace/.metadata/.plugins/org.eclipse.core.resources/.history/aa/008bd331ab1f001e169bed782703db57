...
$('.btn-modify-save').click(function(){
	...
	if(  $(this).text()=='수정'  ){
		...
		//json 데이터를 서버로 보낼때: post, 보내는 내용에 대해 json으로 지정
		$.ajax({
			type: 'post',
...
function modifyStatus( _content ){
	...
	_content.find('.btn-delete-cancel').text('취소')
...
// 	_content.find('.writing').text( comment.length );   //실제입력글자수 표현
...
function stayStatus( _content ){
	...
	_content.find('.btn-modify-save').text('수정')
			.addClass('btn-outline-info').removeClass('btn-primary');
	_content.find('.btn-delete-cancel').text('삭제')
			.addClass('btn-outline-danger').removeClass('btn-secondary');
